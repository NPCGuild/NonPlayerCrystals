...
/// @title   Non Player Crystals
/// @notice This contract mints  Crystals for NPCs and provides
/// administrative functions to the NPCGuild. It allows:
/// * NPCS holders to claim  Crystals
/// * A Guild to set seasons for new opportunities to claim  Crystals
/// * A Guild to form Crystals for use within the Loot ecosystem
/// @custom:unaudited This contract has not been audited. Use at your own risk.
contract Crystals is Context, Ownable, ERC20 {
    // Loot contract is available at https://etherscan.io/address/0xDbCa83E0c066566488B7F58F34cCff203f28D4Eb
    address public lootContractAddress =
        0xDbCa83E0c066566488B7F58F34cCff203f28D4Eb;
    IERC721Enumerable public lootContract;

    // Give out 1376387500000000000 Crystals for every NPC that a user holds
    uint256 public CrystalsPerTokenId = 1376387500000000000 * (10**decimals());
    ...
    */
    ...
    constructor() Ownable() ERC20("CRYSTALS", "NPC") {
        // Transfer ownership to the NPC Guild
        ...
        transferOwnership(0x28C1260be4AB335708e3fF5B7224D5662DD4Ca3e);
        lootContract = IERC721Enumerable(lootContractAddress);
    }

    /// @notice Claim  Crystals for a given LootNPC ID
    /// @param tokenId The tokenId of the NPC NFT
    ...
    /// @notice Claim  Crystals for all tokens owned by the sender
    ...
    /// @notice Claim  Crystals for all tokens owned by the sender within a
    /// given range
    ...
    function _claim(uint256 tokenId, address tokenOwner) internal {
        ...
        // Check that  Crystals have not already been claimed this season
        // for a given tokenId
        require(
            !seasonClaimedByTokenId[season][tokenId],
            "Crystals_CLAIMED_FOR_TOKEN_ID"
        );

        // Effects

        // Mark that  Crystals has been claimed for this season for the
        // given tokenId
        seasonClaimedByTokenId[season][tokenId] = true;

        // Interactions

        // Send  Crystals to the owner of the token ID
        _mint(tokenOwner, CrystalsPerTokenId);
    }
    ...
    /// @notice Allows the DAO to set a season for new  Crystals claims
    /// @param season_ The season to use for claiming Loot
    function daoSetSeason(uint256 season_) public onlyOwner {
        season = season_;
    }

    /// @notice Allows the DAO to set the amount of  Crystals that is
    /// claimed per token ID
    /// @param CrystalsDisplayValue The amount of Loot a user can claim.
    ...
    /// 100 * 10^18.
    function daoSetCrystalsPerTokenId(uint256 CrystalsDisplayValue)
        public
        onlyOwner
    {
        CrystalsPerTokenId = CrystalsDisplayValue * (10**decimals());
    }

    /// @notice Allows the DAO to set the season and  Crystals per token ID
    /// in one transaction. This ensures that there is not a gap where a user
    /// can claim more  Crystals than others
    /// @param season_ The season to use for claiming loot
    /// @param CrystalsDisplayValue The amount of Loot a user can claim.
    /// This should be input as the display value, not in raw decimals. If you
    /// want to mint 100 Loot, you should enter "100" rather than the value of
    ...
    /// Crystals variables directly. It is better practice for security,
    ...
    function daoSetSeasonAndCrystalsPerTokenID(
        uint256 season_,
        uint256 CrystalsDisplayValue
    ) external onlyOwner {
        daoSetSeason(season_);
        daoSetCrystalsPerTokenId(CrystalsDisplayValue);
    }
}
